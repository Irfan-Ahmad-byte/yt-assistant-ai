...
def _get_node_import_query(baseEntityLabel: bool, include_source: bool, entity_label:str) -> str:
    if baseEntityLabel:
        return (
            f"{include_docs_query if include_source else ''}"
            "UNWIND $data AS row "
            f"MERGE (source:`{entity_label}` {{id: row.id}}) "
...
def _get_rel_import_query(baseEntityLabel: bool, entity_label:str) -> str:
    if baseEntityLabel:
        return (
            "UNWIND $data AS row "
            f"MERGE (source:`{entity_label}` {{id: row.source}}) "
            f"MERGE (target:`{entity_label}` {{id: row.target}}) "
...
class Neo4jGraph(GraphStore):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def __init__(
        ...
        self.BASE_ENTITY_LABEL = BASE_ENTITY_LABEL if BASE_ENTITY_LABEL else "__Entity__"
        # Verify connection
        try:
    ...
    def refresh_schema(self) -> None:
        ...
        node_properties = [
            ...
            for el in self.query(
                node_properties_query,
                params={"EXCLUDED_LABELS": EXCLUDED_LABELS + [self.BASE_ENTITY_LABEL]},
            )
        ]
        rel_properties = [
            el["output"]
    ...
    def add_graph_documents(
        ...
        if baseEntityLabel:  # Check if constraint already exists
            constraint_exists = any(
                [
                    el["labelsOrTypes"] == [self.BASE_ENTITY_LABEL]
            ...
            if not constraint_exists:
                # Create constraint
                self.query(
                    f"CREATE CONSTRAINT IF NOT EXISTS FOR (b:{self.BASE_ENTITY_LABEL}) "
                    "REQUIRE b.id IS UNIQUE;"
                )
                self.refresh_schema()  # Refresh constraint information

        node_import_query = _get_node_import_query(baseEntityLabel, include_source, self.BASE_ENTITY_LABEL)
        rel_import_query = _get_rel_import_query(baseEntityLabel, self.BASE_ENTITY_LABEL)

        # Split graph_documents into chunks for parallel processing
        num_threads = threading.active_count()
        if len(graph_documents) < num_threads:
            chunks = [graph_documents[i::num_threads] for i in range(num_threads)]
        else:
            chunks = [graph_documents]

        # Create and start threads
        threads = []
        for chunk in chunks:
            t = threading.Thread(target=self.process_chunk, args=(chunk, node_import_query, rel_import_query))
            t.start()
            threads.append(t)
    ...
    def process_chunk(self, chunk: List[GraphDocument], node_import_query: str, rel_import_query: str) -> None:
